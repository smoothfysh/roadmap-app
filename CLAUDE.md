...
| CSV helpers | `parseCsvLine`, `splitCsvRecords`, `csvToItems`, `itemsToCsv`, `downloadCsv` |
...
- **Tag auto-detection**: Titles prefixed `FR: …`, `DE/AT: …`, `V1: …`, `PHASE 2: …` etc. split into `tag` + `text` at save time. **Not country-specific** — `extractTag` takes whatever precedes the first colon, upper-cases it (`v1:` → `V1`), and accepts letters/digits with spaces or `/` as separators. Two guards, both deliberate: `TAG_MAX_LEN` (12) keeps a long prefix from becoming a card-breaking badge, and a `//` after the colon is treated as a URL. The user knowingly accepted the flip side — `Note: …` tags as `[NOTE]` and `09:00 standup` as `[09]` — so **don't "fix" those as bugs** (widened in 4.37.0 from the old 2–3-uppercase-letter country rule). Badge colours are in the `TAG_STYLES` constant — easy to extend; unlisted slash combos get `TAG_COMBO_STYLE`, everything else the white outline pill.
...
- **Card indicator cluster**: the collapsed card header's right-hand furniture is deliberately **capped at two always-visible markers** — the generic links badge and the milestones ◆ — plus the grip and delete button, which are hover-only. This is a budget, not an accident: every marker is fixed-width and comes straight off the title, and the user rejected a four-marker cluster as clutter (4.36.0). Adding a third means removing one. Removed to make room: the 6px grey "has notes" dot, and the clickable status dot (status is now set only in the item modal's footer picker — the card's background tint via `flagStyles` already *displays* the flag, so only the shortcut was lost). The ◆ is hollow while any milestone is still ahead and solid once all have passed, matching the Gantt bars; its popover is absolutely positioned inside the card (`bottom-full right-0`) rather than in a fixed overlay — the board columns don't clip like the Gantt's `overflow-x-auto` body does — and dismisses on outside click or `Esc`.
...
- **CSV records are quote-aware**: `csvToItems` splits via `splitCsvRecords`, which breaks a record only on a newline at quote depth zero. **Never go back to `csvText.split(/\r?\n/)`** — descriptions are free text and routinely contain paragraph breaks, which `itemsToCsv` correctly quotes on export. The naive split tore those records in half and nulled every field after `description`, so the app's own Export CSV produced files its Import CSV mangled (fixed 4.34.0). `parseCsvLine` handles a whole record unchanged; a newline inside quotes is just another character to it.
...
- `public/sample-roadmap.json` / `public/sample-roadmap.csv` are the try-me fixtures behind the burger menu's **Download sample JSON / CSV**, for testing Restore JSON and Import CSV. A fictional apparel platform ("Threadhouse"), 102 items over 9 workstreams — 3–6 per lane in DONE and COMING SOON, deliberately uneven, so the board looks like a real team's load rather than a demo stub. Written to hit every feature: all five flags, all four strategic categories, every outcome type, all three tag-style branches (known `DE`, known combo `FR/DE`, unlisted combo `ES/IT`, unknown single `NL`), every date-pill format, milestones, unscheduled items, a markdown summary, ten link providers plus an unrecognised host, and one item still on legacy `jiraUrl`/`confluenceUrl`. **Only the JSON is hand-edited** — regenerate the CSV with `node scripts/build-sample-csv.mjs`, which reuses the app's own `CSV_ITEM_HEADERS` and escaping. Links go through `import.meta.env.BASE_URL`, not a leading slash, so they resolve under the company build's `--base=./`.
